...
internal class CommonBigInteger : BigInteger {
  ...
  constructor(other: ByteArray, off: Int = 0, len: Int = other.size) {
    ...
    } else if (off < 0 || off >= other.size || len < 0 ||
      len > other.size - off
    ) { // 0 <= off < val.length
  ...
  private fun squareKaratsuba(): CommonBigInteger {
    ...
    return xhs.shl(half * 32).plus(xl.plus(xh).square().minus(xhs.plus(xls))).shl(half * 32)
      .plus(xls)
  }
...
//        return result
  ...
  private fun divideBurnikelZiegler(other: CommonBigInteger): CommonBigInteger {
    return divideAndRemainderBurnikelZiegler(other)[0]
  }
  ...
  private fun remainderBurnikelZiegler(other: CommonBigInteger): CommonBigInteger {
    return divideAndRemainderBurnikelZiegler(other)[1]
  }
  ...
  override fun modPow(exponent: BigInteger, modulus: BigInteger): CommonBigInteger {
    ...
    if (modulus.testBit(0)) { // odd modulus
      ...
      if (modulus._mag.size < MAX_MAG_LENGTH / 2) {
        result = a1.times(m2).times(y1).plus(a2.times(m1).times(y2)).rem(modulus)
      } else {
  ...
  private fun oddModPow(y: CommonBigInteger, z: CommonBigInteger): CommonBigInteger {
    ...
    val n0 =
      (mod[modLen - 1].toLong() and LONG_MASK) + (mod[modLen - 2].toLong() and LONG_MASK shl 32)
  ...
  companion object {
    ...
    @JsName("of")
    @JvmStatic
    fun of(value: Int): CommonBigInteger {
    ...
    private fun sum(x: IntArray, other: Long): IntArray {
      ...
      if (highWord == 0) {
        ...
        if (xIndex == 1) {
          ...
          result[xIndex] = sum.toInt()
          sum =
            (x[--xIndex].toLong() and LONG_MASK) + (highWord.toLong() and LONG_MASK) + sum.ushr(32)
          result[xIndex] = sum.toInt()
        }
      }
    ...
    private fun subtract(other: Long, little: IntArray): IntArray {
      ...
      if (highWord == 0) {
        ...
        if (little.size == 1) {
          val difference =
            (other.toInt().toLong() and LONG_MASK) - (little[0].toLong() and LONG_MASK)
          ...
          var difference =
            (other.toInt().toLong() and LONG_MASK) - (little[1].toLong() and LONG_MASK)
          result[1] = difference.toInt()
          difference =
    ...
    internal var bnExpModThreshTable =
      intArrayOf(7, 25, 81, 241, 673, 1793, Int.MAX_VALUE) // Sentinel
