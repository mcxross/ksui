...
internal open class MutableBigInteger {
  ...
  private fun mulsub(q: IntArray, a: IntArray, x: Int, len: Int, offset: Int): Int {
    ...
    for (j in len - 1 downTo 0) {
      ...
      carry = product.ushr(32) + if (difference and LONG_MASK > product.toInt().inv()
          .toLong() and LONG_MASK)
        1
      else
        0
    }
  ...
  fun multiply(y: MutableBigInteger, z: MutableBigInteger) {
    ...
    run {
      ...
      while (j >= 0) {
        val product =
          (y.value[j + y.offset].toLong() and LONG_MASK) * (value[xLen - 1 + offset].toLong() and LONG_MASK) + carry
    ...
    for (i in xLen - 2 downTo 0) {
      ...
      while (j >= 0) {
        val product =
          (y.value[j + y.offset].toLong() and LONG_MASK) * (value[i + offset].toLong() and LONG_MASK) +
  ...
  fun divideOneWord(divisor: Int, quotient: MutableBigInteger): Int {
    ...
    var xlen = intLen
    while (--xlen > 0) {
      val dividendEstimate =
        remLong shl 32 or (value[offset + intLen - xlen].toLong() and LONG_MASK)
      val q: Int
  ...
  private fun mulsubLong(q: IntArray, dh: Int, dl: Int, x: Int, offset: Int): Int {
    ...
    q[offset--] = difference.toInt()
    var carry = product.ushr(32) + if (difference.toLong() and LONG_MASK > product.inv()
        .toLong() and LONG_MASK)
      1
    else
      0
    ...
    q[offset--] = difference.toInt()
    carry = product.ushr(32) + if (difference.toLong() and LONG_MASK > product.inv()
        .toLong() and LONG_MASK)
      1
    else
      0
