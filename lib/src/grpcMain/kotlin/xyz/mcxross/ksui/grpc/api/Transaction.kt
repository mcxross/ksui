...
internal class Transaction(private val runtime: GrpcRuntime) : TransactionProtocol {
  override suspend fun getTransaction(
    ...
    handleGrpc { internalGetTransaction(runtime, request) }

  override suspend fun getTransaction(
    digest: String,
    readMask: FieldMask?,
  ): Result<GetTransactionResponse, SuiError> =
    handleGrpc { internalGetTransaction(runtime, digest, readMask) }

  override suspend fun batchGetTransactions(
    digests: List<String>,
    readMask: FieldMask?,
  ): Result<BatchGetTransactionsResponse, SuiError> =
    handleGrpc { internalBatchGetTransactions(runtime, digests, readMask) }

  override suspend fun executeTransaction(
    request: ExecuteTransactionRequest
  ): Result<ExecuteTransactionResponse, SuiError> =
    handleGrpc { internalExecuteTransaction(runtime, request) }
  ...
  override suspend fun executeTransaction(
    ...
    handleGrpc { internalExecuteTransaction(runtime, transaction, signatures, readMask) }
  ...
  override suspend fun executeTransaction(
    ...
    handleGrpc { internalExecuteTransaction(runtime, transactionBytes, signatures, readMask) }

  override suspend fun simulateTransaction(
    request: SimulateTransactionRequest
  ): Result<SimulateTransactionResponse, SuiError> =
    handleGrpc { internalSimulateTransaction(runtime, request) }
