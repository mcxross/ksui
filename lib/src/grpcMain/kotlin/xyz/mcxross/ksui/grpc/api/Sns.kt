...
internal class Sns(private val runtime: GrpcRuntime) : SnsProtocol {
  ...
  ): Result<LookupNameResponse, SuiError> =
    handleGrpc { internalLookupName(runtime, request) }

  override suspend fun lookupName(name: String): Result<LookupNameResponse, SuiError> =
    handleGrpc { internalLookupName(runtime, name) }

  override suspend fun reverseLookupName(
    request: ReverseLookupNameRequest
  ): Result<ReverseLookupNameResponse, SuiError> =
    handleGrpc { internalReverseLookupName(runtime, request) }

  override suspend fun reverseLookupName(
    address: String
  ): Result<ReverseLookupNameResponse, SuiError> =
    handleGrpc { internalReverseLookupName(runtime, address) }
