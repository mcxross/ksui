...
class ProgrammableTransactionResolutionTest :
  StringSpec({
    val bcs = Bcs

    "PTB builder resolves ObjectStr inputs before serialization" {
      val ptb =
        buildResolvedPtb(FakeResolver(mutableInputIndexes = setOf(0))) {
          ...
          val params = moveCall {
            ...
            arguments =
              listOf(pure("title"), pure("description"), pure(listOf(0UL)), pure(900000UL), clock())
          }
...
private suspend fun buildResolvedPtb(
  resolver: ProgrammableTransactionResolver,
  block: PtbDsl.() -> Unit,
): ProgrammableTransaction {
  val builder = ProgrammableTransactionBuilder()
  PtbDsl(builder).block()
  return builder.build(resolver)
}

private class FakeResolver(private val mutableInputIndexes: Set<Int>) :
  ProgrammableTransactionResolver {
  override suspend fun mutableInputIndexes(commands: List<Command>): Set<Int> = mutableInputIndexes

  override suspend fun resolveObjects(objectIds: List<String>): Map<String, ResolvedObject> =
    objectIds.associateWith { objectId ->
      ResolvedObject(
        objectId = objectId,
        version = 1L,
        digest = "BqrFumAb7yjZKKQUApKjzqdgHUZY3XaTNWxNQsvjMSBc",
        owner = ResolvedObjectOwner.AddressOwner,
      )
    }
}
