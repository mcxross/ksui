 ...
 */
    ...
    @Throws(SigningException::class) fun sign(msg: String?): String {
 ...
 */
        @Throws(SignatureSchemeNotSupportedException::class) fun decodeBase64(encoded: String?): SuiKeyPair<*> {
             val keyPairBytes = Base64.decode(encoded)
            
             val scheme = SignatureScheme.valueOf(keyPairBytes[0])
?: throw SignatureSchemeNotSupportedException()
            return when (scheme) {
        SignatureScheme.ED25519 -> ED25519KeyPair.decodeBase64(keyPairBytes)
        SignatureScheme.Secp256k1 -> SECP256K1KeyPair.decodeBase64(keyPairBytes)
