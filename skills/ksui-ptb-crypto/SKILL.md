...
description: "Workflow for Ksui programmable transaction blocks, accounts, signatures, private keys, BCS serialization, passkeys, sponsored transactions, and platform crypto. Use when editing ptb, account, core/crypto, dsl/SponsoredPtb, transaction signing/execution, serializers for transaction data, or tests around PTB building and signing."
---
...
Read [references/ptb-crypto-map.md](references/ptb-crypto-map.md) for the transaction, account, crypto, platform, and test file map.
...
5. Add focused unit tests before e2e tests for builder shape, serialization, key import/export, signature verification, and option/filter conversion.
6. Run `./gradlew :ksui:jvmTest` after local changes; add platform compile tasks when touching non-JVM actual implementations.
